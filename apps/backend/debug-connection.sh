...
if [ "$(docker ps -q -f name=flash-me-backend)" ]; then
...
docker inspect flash-me-backend -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'
echo ""

echo "Checking if server is listening inside container..."
docker exec flash-me-backend sh -c "netstat -tulpn | grep LISTEN" || docker exec flash-me-backend sh -c "ps aux | grep node"

echo "Checking port mapping..."
docker port flash-me-backend

echo "Testing connection from inside the container..."
docker exec flash-me-backend sh -c "wget -q -O- http://localhost:3000/health || echo 'Failed to connect internally'"
...
echo "3. Check the logs: docker logs flash-me-backend"
